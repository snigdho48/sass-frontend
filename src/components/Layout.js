...
import { setPageLoading, setNavigationLoading, setSidebarOpen, toggleSidebar } from '../store/slices/uiSlice';
...
const Layout = () => {
  ...
  const { sidebarOpen, navigationLoading } = useSelector((state) => state.ui);
  ...
  useEffect(() => {
    if (isAuthenticated && user && navigationLoading) {
      const timer = setTimeout(() => dispatch(setNavigationLoading(false)), 100);
              ...
              <div className='relative'>
                <div className='flex items-center gap-x-2 sm:gap-x-3'>
                  <div className='flex flex-col text-right min-w-0'>
                    <span
                      className='text-xs sm:text-sm font-medium text-gray-900 dark:text-gray-100 truncate max-w-[100px] sm:max-w-[120px] lg:max-w-none'
                      title={user?.get_full_name || user?.email}
                    >
                      {(() => {
                        const fullName =
                    ...
                    <span className='text-xs text-gray-500 dark:text-gray-400 hidden sm:block'>
                      {user?.role_display
                        ?.replace("General User", "User")
                        .replace("Administrator", "Admin")
                        .replace("Super Administrator", "Super Admin") ||
                        user?.role}
                    </span>
                    <span
                      className='text-xs text-gray-500 dark:text-gray-400 sm:hidden truncate max-w-[100px]'
                      title={user?.role_display || user?.role}
                    >
                      {user?.role_display
                        ?.replace("General User", "User")
                        .replace("Administrator", "Admin")
                  ...
                  <div
                    className={`h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-primary-600 flex items-center justify-center flex-shrink-0 ${
                      user?.profile_picture ? "hidden profile-fallback" : ""
                    }`}
                  >
