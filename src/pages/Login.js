...
import { login as loginThunk, clearError, resetLoading } from '../store/slices/authSlice';
...
const Login = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      const isSuperAdmin = user.is_super_admin || user.role === 'super_admin';
      const isInactive = !isSuperAdmin && (user.is_admin || user.is_general_user) && !user.is_active;
      if (isInactive && !showInactiveModal) setShowInactiveModal(true);
    }
  }, [isAuthenticated, user, showInactiveModal]);

  useEffect(() => {
    return () => { dispatch(clearError()); dispatch(resetLoading()); };
  }, [dispatch]);
