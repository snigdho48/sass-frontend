...
const DataEntry = () => {
  ...
  const createEntryMutation = useMutation(dataService.createDataEntry, {
    onSuccess: async () => {
      ...
      await queryClient.refetchQueries(['entries']);
  ...
  const createPlantMutation = useMutation(dataService.createPlant, {
    onSuccess: async () => {
      ...
      await queryClient.refetchQueries(['plants-management']);
      setPlantFormData({
        name: '',
        is_active: true,
        owner_id: null,
  ...
  const createWaterSystemMutation = useMutation(dataService.createWaterSystem, {
    onSuccess: async () => {
      ...
      await queryClient.refetchQueries(['water-systems']);
      await queryClient.refetchQueries(['water-systems-all']);
                  ...
                  <div className='border-t border-gray-200 dark:border-gray-700 pt-4 sm:pt-6'>
                    ...
                    <div className='space-y-2 sm:space-y-3'>
                      <label className='block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>
                        ...
                        <div className='text-sm text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-md px-3 py-2'>
                          No Admin users in the system. Create an Admin user from User Management first, then assign them as plant owner here.
                        </div>
                      ) : (
                        <SearchableUserSelect
                          options={Array.isArray(adminUsers) ? adminUsers : []}
